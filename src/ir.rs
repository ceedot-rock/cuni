...
//! PCC remains lossless compressor only.
//!
//! Universal AST v0 oddity policy (portable core): prefer **map** (exact) or
//! **hard-fail** refuse seats — never approximate / soft rewrite when seats diverge.
//! Relates lightly to `src/oddity.rs` (M1 diagnostics) for future refuse codes.
//!
...
//! | `src/oddity.rs` | M1 hard-fail diagnostics — future IR validator refuse table input |
//!
...
/// Universal AST v0 oddity-matrix policy for a portable construct (sketch).
/// Prefer **Map** (exact across seats) or **HardFail**; never Approximate.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OddityPolicy {
    /// Exact portable mapping — same gold observation on native seats.
    Map,
    /// Refuse — cannot prove identical gold (no approximate mode).
    HardFail,
}
...
mod tests {
    ...
    #[test]
    fn oddity_policy_has_no_approximate_seat() {
        // Compile-time honesty: only Map | HardFail exist on OddityPolicy.
        let map = OddityPolicy::Map;
        let hard = OddityPolicy::HardFail;
        assert_ne!(map, hard);
        assert_eq!(format!("{map:?}"), "Map");
        assert_eq!(format!("{hard:?}"), "HardFail");
    }
